...
layui.use(['jquery', 'form', 'layedit', 'flow', 'laytpl'], function () {
    ...
    //评论显示
    function showmsg(aid) {
        $("#blog-comment").children().remove();
        flow.load({
            elem: "#blog-comment",
            isAuto: true,
            end: "没有更多了哦！╮(╯▽╰)╭",
    ...
    form.on('submit(formLeaveMessage)', function (data) {
        var loading = layer.msg("正在提交评论...", {
            icon: 16,
            time: 0
        });
        console.log(data.field.aid);
...
function nextpage(elem) {
    ...
    $.get("https://api.icqcore.com//api/Message/Replypage", { "pageindex": page, "rootid": rootid,"aid":aid }, function (result) {
